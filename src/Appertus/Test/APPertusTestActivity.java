...
public class APPertusTestActivity extends Activity {
    ...
    public void UpdateSlidePosText(){
	...
	URL CameraUrl = null;
	String camera_url = "http://" + mIPField.getText() + "/ElphelVision/elphelvision_interface.php";
	try {
	...
	try {
		...
		for (int s = 0; s < nodeLst.getLength(); s++) {
			...
			CameraImageWidth = Integer.parseInt(((Node) fstNm.item(0)).getNodeValue());

			NodeList lstNmElmntLst = fstElmnt.getElementsByTagName("image_height");
			Element lstNmElmnt = (Element) lstNmElmntLst.item(0);
			NodeList lstNm = lstNmElmnt.getChildNodes();
			CameraImageHeight = Integer.parseInt(((Node) lstNm.item(0)).getNodeValue());
		    }
		}
		// Create final Text
		String Content = "Image Width: " + CameraImageWidth + "\n";
		Content += "Image Height: " + CameraImageHeight + "\n";
		mTextresult.setText(Content);
	    } catch (Exception e) {
		e.printStackTrace();
